...
- **首页**：公司中英文名称、品牌 Slogan、成立日期、动态银白光带背景，以及服务与合作入口。
- **关于零一**：公司主体、创始人、品牌理念与“定义—构建—成长”的合作方式。
- **产品与服务**：以项目实践折叠画廊展示第二十五小时、零一 API 与零一 AI 日新社，封面使用对应网站的首页截图。
- **联系**：QQ 二维码、技术栈 LogoLoop，以及可跳转的 ICP 与公安备案信息。

视觉采用极简黑白风格，包含滚动玻璃导航、文字描边与立体动效、白—蓝—紫流光区块标题、指针追踪 BorderGlow 内容框、项目折叠画廊和区块入场动画，并提供移动端布局及减少动态效果支持。
...
- 画廊封面来自三个对应网站的首页截图；网站首页明显改版后应同步更新截图。上线前核对二维码和备案信息。
